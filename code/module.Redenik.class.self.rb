...
module Redenik
	class << self
		...
		private
		def _gen_actors
			START___MAX_ACTORS.times{
				rand_class = STATS___CLASSES[STATS___CLASSES.keys.sample]
				@game_actors << Redenik::Actor.new(
					Redenik::NameGen.make_name(3,4),
					rand_class[:class_name],
					[rand_class[:st],rand_class[:dx],rand_class[:iq],rand_class[:ht]],
